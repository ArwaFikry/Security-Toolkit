...
def info():
    messagebox.showinfo('Audio Steganography', 'Successful Process')
...
def update_countdown(event=None):
    ...
    countdown_label.configure(text=f": الحروف المتبقية {remaining_chars}", text_color="white")
...
def display():
    ...
    lb3 = CTkLabel(root, text=": أدخل الرسالة السرية", font=('Helvetica',20,'bold'), bg_color="#000e17", text_color="white")
    lb3.place(x=410, y=65)
    ...
    countdown_label = CTkLabel(root, text=f"الحروف المتبقية{max_chars}", font=('Helvetica', 12,'bold'),
                               bg_color="#01192a")
    countdown_label.place(x=455, y=300)

    text_box.bind("<KeyRelease>", update_countdown)

    lb3 = CTkLabel(root, text=": أختر الملف", font=('Helvetica', 20,'bold'), bg_color='#01192a')
    lb3.place(x=500, y=340)
    ...
    but3 = CTkButton(root, text="تصفح", command=lambda: action("browse_file"), width=50, height=25,
                     font=('Helvetica', 15, 'bold'), corner_radius=25, bg_color='#03002e', fg_color='#0d326e',image=image)
    but3.place(x=200, y=340)

    lb6 = CTkLabel(root, text=": عرض مسار الملف", font=('Helvetica', 20,'bold'), bg_color='#01192a')
    lb6.place(x=500, y=380)

    global ent8
    ent8 = CTkEntry(root, width=180, height=25, fg_color="white", text_color='red')
    ent8.place(x=310, y=380)


    lb4 = CTkLabel(root, text=": أدخل مفتاح التشفير", font=('Helvetica', 20,'bold'), bg_color='#01192a')
    lb4.place(x=500, y=420)

    global ent2
    ent2 = CTkEntry(root, width=180, height=25, fg_color="white", text_color='red')
    ent2.place(x=310, y=420)

    options = ["LSB", "Phase"]
    combo_box = CTkOptionMenu(root, values=options, width=140, bg_color='#011828',font=('Helvetica',13,'bold'))
    combo_box.place(x=475, y=465)

    options = ["تشفير", "فك التشفير"]
    combo_box1 = CTkOptionMenu(root, values=options, width=150, bg_color='#011828',font=('Helvetica',13,'bold'))
    combo_box1.place(x=195, y=465)

    global but4
    but4 = CTkButton(root, text="موافق", command=Apply, width=120, height=30, font=('Helvetica', 15,'bold'),
                     bg_color='#011828', fg_color='#0d326e', corner_radius=25,image=image1)
    but4.place(x=350, y=465)
    ho.CreateToolTip(but4, 'الصوتي المقطع في الاخفاء تنفيذ')
