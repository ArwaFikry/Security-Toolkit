...
from Audiolsb import hide_data_in_audio,reveal_data_from_audio
import hover as ho
...
def action(pointer):
    if pointer == "browse_file":
        global file_path
        file_path = filedialog.askopenfilename(title="Choose a Audio file",
                                                filetypes=(("Audio Files", "*.wav"),))
        if file_path:
            ent1.delete(0,"end")
            ent1.insert('end', file_path)
...
def Apply():
    ...
    if len(Audiofile) > 0 :
        ...
        if process =="Encode" and len(secret) >1:
            if technique == "LSB":
                hide_data_in_audio(Audiofile,secret,key)
                info()
        else:
            text_box.delete(0.0, "end")
            if technique == "LSB":
                secret=reveal_data_from_audio(Audiofile,key)
                text_box.delete(0.0,"end")
...
def display():
    ...
    ho.CreateToolTip(but3, 'browse cover audio file')
    ...
    ho.CreateToolTip(but4, 'processed audio stegno')
